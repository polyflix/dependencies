...
export const Roles = (...roles: Role[]) => SetMetadata(ROLES_REFLECT_KEY, roles);
...
export class RolesGuard implements CanActivate {
    ...
    canActivate(context: ExecutionContext): boolean {
        const requiredRoles = this.reflector.getAllAndOverride<Role[] | undefined>(ROLES_REFLECT_KEY, [
            context.getHandler(),
            context.getClass()
        ])
        ...
        this.logger.debug("[" + path + "]" +"Expected roles [" + requiredRoles + "], given roles [" + roles + "], authorized: " + hasRoles)
    ...
    fetchRoleHeader(context: ExecutionContext): string {
        try {
            return context.switchToHttp().getRequest().headers[RolesGuard.USER_ROLES_HEADER];
        } catch(e) {
            return ""
        }
    }
